...
def image_generator_create(
    ...
    Returns:
        Image metadata dict or None if generation fails
    """
    try:
        from content.image_orchestrator import generate_image_for_content

        # Prepare analysis report dict
        analysis_dict = {
            "instrument": report.instrument,
            "current_price": report.raw_data.get("current_price") if hasattr(report, 'raw_data') else None,
            "change_pct": report.raw_data.get("price_change_pct") if hasattr(report, 'raw_data') else None,
...
def run_pipeline(
    ...
    if not skip_content and not skip_images and commentary:
        try:
            image_result = image_generator_create(report, commentary)
            if image_result and image_result.get("success"):
                image_path = image_result.get("image_path")
                # Add image metadata to commentary result
